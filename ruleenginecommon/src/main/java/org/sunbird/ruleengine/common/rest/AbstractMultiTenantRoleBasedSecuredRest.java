...
import org.sunbird.ruleengine.model.ControllerConfig;
import org.sunbird.ruleengine.service.AbstractClientService;
import org.sunbird.ruleengine.service.GenericService;

public abstract class AbstractMultiTenantRoleBasedSecuredRest<T extends AbstractMultiTenantEntity, U extends AbstractMultiTenantEntity> extends GenericMultiTenantRoleBasedSecuredRest<T, U>{
	private static final Logger logger = LogManager.getLogger(AbstractMultiTenantRoleBasedSecuredRest.class);
	private Class<T> t;
	private Class<U> u;
	...
	public AbstractMultiTenantRoleBasedSecuredRest() {
	}
	
	public AbstractMultiTenantRoleBasedSecuredRest(Class<T> t, Class<U> u) {
		this.t=t;
		this.u=u;
	}
	...
	public @ResponseBody Response<T> update(@PathVariable("clientCode") String clientCode,@RequestBody T t, Principal principal){
		...
		if(isUpdateAuthorized() && getService().getCount(instantiateCriteria(), getAuthorizationQueryForUpdate(t, principal)).longValue()==0)
		{
			throw new NotAuthorizedException();
		}
		if(principal!=null)
	...
	public @ResponseBody Response<T> deleteWithBiginteger(@PathVariable("clientCode") String clientCode,@PathVariable("id") BigInteger id, Principal principal){
...
public String getTableForUpdateByColumn(){return null;}
	...
	public  String getAuthorizationQueryForDelete(BigInteger id, Principal principal){return "select count(id) from "+getUserTable()+" usr inner join user_role_join ur on usr.id=ur.user_id inner join role_permission_join rp on rp.role_id=ur.role_id inner join permission pr on pr.id=rp.permission_id where pr.permission_name="+deletePermissionName()+" and usr.id="+principal.getName()+" ";};

...
@Component
public class NashornStepProcessor implements StepProcessor {
		...
		try {
			...
			try {
				...
				if (step.getResponseFormatterJs() != null && !step.getResponseFormatterJs().trim().isEmpty()) {
					Object response = nashornHelper.process(step.getResponseFormatterJs(), objectHolder,
							objectHolder);
					if  ( step.getSuccessKey() == null || step.getSuccessKey().isEmpty() || templateParser.parse(step.getClientId().toString(), "success_key_template_" + step.getId().toString(),step.getSuccessKey(), objectHolder)
						...
						saveStepHistory(step, objectHolder, JsonUtil.toString(objectHolder), response, null,
								Status.FAILURE);
						failOverHandler(jobDetail, true, step, objectHolder, null, response.toString(), level);
					}
				}
			} catch (Throwable e) {
				e.printStackTrace();
